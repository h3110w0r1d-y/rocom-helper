...
- 服务端 IP/主机名
- 服务端 HTTP 端口
- 从服务端用户列表选择的 `uid`；默认选择会持久化

## 用户列表

每次启动或点击连接时，插件会先读取用户列表：

```text
GET /api/users
```

响应为 `{uid, name, avatar}` 对象数组。插件恢复上次保存的 UID；若该 UID 已不存在，则选择列表第一项并更新默认 UID。列表为空或请求失败时不会继续版本校验、地图快照和 SSE 连接。

## 服务端版本

用户列表读取并选定 UID 后，客户端读取服务端版本：
...
持久标注的新增、拖动、删除都提交到服务端，由服务端写库并通过 SSE 实时推送给客户端；持久标注初始状态以 `/api/map-markers` 全量快照为准。
